...
#        print(df.columns)
    ...
    @staticmethod
    def plotTrainingLoss(modelFit: any, titlePlot: str):
        '''
        plot del modello dopo training
        $args$ modelFit è una matrice numpy
        $args$ titlePlot è una stringa titolo del grafico
        ...
        plt.plot(modelFit.history["val_loss"], label="Validation Loss")
        plt.title(titlePlot)
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend()
    ...
    @staticmethod
    def plotConfusionMatrix(y_true, y_pred_classes, model_name: str):
    ...
    # Scrive i numeri dentro le celle
        for i in range(cm.shape[0]):
            for j in range(cm.shape[1]):
                plt.text(j, i, format(cm[i, j], 'd'), ha="center", va="center", color="white")

        plt.ylabel("True label")
        plt.xlabel("Predicted label")
        titlePlot = "Confusion Matrix - " + model_name
...
class modelAnalysis(object):
    ...
    @staticmethod
    def plotDisagreement(x_train_ds):
        m = x_train_ds["margine"].values
        H = x_train_ds["entropy"].values

        # normalizzazione entropy
        H_norm = H / np.max(H)

        # disagreement
        disagreement = np.abs(H_norm - (1-m))

        plt.figure(figsize=(6,5))

        sc = plt.scatter(m, H, c=disagreement, s=8)

        plt.xlabel("Margin")
        plt.ylabel("Entropy")
        plt.title("Epistemic disagreement between uncertainty measures")

        plt.colorbar(sc,label="Uncertainty disagreement")

        plt.show()
        return plt
    
    @staticmethod
    def plotUnspace(x_train_ds, alpha_m, beta_m, beta_h):
        m = x_train_ds["margine"]
        H = x_train_ds["entropy"]
        bnd = x_train_ds["bnd3wd"]
        ...
        plt.scatter(m[POS], H[POS], s=5, alpha=0.4, label="POS/NEG")
        plt.scatter(m[BND], H[BND], s=10, color="orange", label="BND")
        ...
        plt.xlabel("Margin")
        plt.ylabel("Entropy")
        plt.title("3WD Decision Regions")
        plt.legend()
